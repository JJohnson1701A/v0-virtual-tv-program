...
export function VirtualTVDisplay({
  ...
  // =========================================================================
  //  Set block end epoch from the hook's pre-computed value
  // =========================================================================
  useEffect(() => {
    if (!media) return
    if (media.blockEndEpoch) {
      blockEndRef.current = media.blockEndEpoch
    } else {
      // Fallback: compute from schedule times (for blocks/marathons that don't set it)
      const now = new Date()
      const end = parseScheduleTime(media.endTime)
      const blockEnd = new Date(now)
      blockEnd.setHours(end.hours24, end.minutes, 0, 0)
      // Only add a day for genuine overnight wraps, not same-day blocks
      const start = parseScheduleTime(media.startTime)
      const startDate = new Date(now)
      startDate.setHours(start.hours24, start.minutes, 0, 0)
      if (blockEnd.getTime() <= startDate.getTime()) {
